...
public abstract class AbstractEventParser<EVENT> extends AbstractCanalLifeCycle implements CanalEventParser<EVENT> {
    ...
    // 操作类型过滤
    protected String operFilter;
    protected List operFilterList;
	...
	public void setFieldFilter(String fieldFilter) {
        logger.warn("FieldFilter: "+fieldFilter);
    ...
    public String getOperFilter() {
        return operFilter;
    }

    public void setOperFilterList(List operFilterList) {
        this.operFilterList = operFilterList;
    }

    public void setOperFilter(String operFilter) {
	    logger.warn("operFilter: " + operFilter);
        this.operFilter = operFilter;
        this.operFilterList = new ArrayList<>();
        String[] operFilters = operFilter.toUpperCase().trim().split(",");
        for (int i = 0; i < operFilters.length; i++) {
            if (operFilters[i].equals("INSERT")) {
                this.operFilterList.add(LogEvent.WRITE_ROWS_EVENT);
            }
            if (operFilters[i].equals("DELETE")) {
                this.operFilterList.add(LogEvent.DELETE_ROWS_EVENT);
            }
            if (operFilters[i].equals("UPDATE")) {
                this.operFilterList.add(LogEvent.UPDATE_ROWS_EVENT);
            }
        }
    }

    public List getOperFilterList() {
        return operFilterList;
    }
}
